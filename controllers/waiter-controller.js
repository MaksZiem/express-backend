...
exports.getUserOrdersWithTips = async (req, res, next) => {
  ...
  try {
    ...
    // 1. Znalezienie zamówień użytkownika
    ...
    // 2. Pobieramy wszystkie napiwki powiązane z zamówieniami użytkownika
    const orderIds = userOrders.map((order) => order._id);
    const tips = await Tip.find({ order: { $in: orderIds }, user: userId });

    // 3. Przetwarzamy zamówienia, aby dodać napiwki
...
exports.postCreateOrder = async (req, res, next) => {
    ...
    try {
      // Znalezienie stolika na podstawie numeru
      const table = await Table.findOne({ number: tableNumber }).populate(
        "dishCart.items.dishId"
      );
      ...
      if (!orderDate) {
        ...
        // Formatujemy datę z godziną, minutą i sekundą
        newOrderDate = `${month}.${day}.${year} ${hours}:${minutes}:${seconds}`;
      }
...
//dzialajace dobrze aele bez sekund
